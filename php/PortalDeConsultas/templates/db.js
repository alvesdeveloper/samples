    ...
    function applyFilter(data, filter) {
        return $.grep(data, function(client) {
            return (!filter.nome || client.nome.indexOf(filter.nome) > -1)
                && (filter.cpf === undefined || client.cpf === filter.cpf)
                && (!filter.NumeroTitulo || client.NumeroTitulo.indexOf(filter.NumeroTitulo) > -1)
                && (!filter.Valor || client.Valor === filter.Valor);
        });
    };
    ...
    var db = {
        ...
        loadData: function (filter) {
            var data;
             return $.ajax({
                 type: "GET",
                 url: "http://192.168.43.130:80/portaldeconsultas/templates/repositorio.php",
                 data: filter,
